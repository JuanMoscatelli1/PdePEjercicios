...
materiaPesada(Materia):-integradora(Materia), materia(Materia,6).
...
esCorrelativa(administracionGeneral,Materia):-
    esCorrelativa(administracionDeRecursos,Materia).
esCorrelativa(administracionDeRecursos,Materia):-
    esCorrelativa(disenioDeSistemas,Materia).
esCorrelativa(disenioDeSistemas,Materia):-
    esCorrelativa(analisisDeSistemas,Materia).
esCorrelativa(analisisDeSistemas,Materia):-
    esCorrelativa(sistemasYOrganizaciones,Materia).
esCorrelativa(fisicaII,Materia):-
    esCorrelativa(fisicaI,Materia).
...
esCorrelativa(teoriaDeControl,Materia):-
    esCorrelativa(mateSuperior,Materia).
esCorrelativa(mateSuperior,Materia):-
    esCorrelativa(analisisMatematico2,Materia).
esCorrelativa(analisisMatematico2,Materia):-
    esCorrelativa(analisisMatematico1,Materia).
esCorrelativa(analisisMatematico2,Materia):-
    esCorrelativa(algebraYGeometriaAnalitica,Materia).

materiaQueHabilita(comunicaciones,redesDeInformacion).
materiaQueHabilita(comunicaciones,Materia):-materiaQueHabilita(redesDeInformacion,Materia).
materiaQueHabilita(redesDeInformacion,proyectoFinal).
...
casoSinDescanso(Alumno,Materia):-curso(Alumno,Materia,_,cuatrimestral(Anio,2)),
                        curso(Alumno,Materia,_,cuatrimestral(Anio2,1)), Anio2 is Anio + 1.
...
invictus(Alumno):- curso(Alumno,_,_,_),(recursoMateria(Alumno,_)).
...
buenaCursada(Alumno):- curso(Alumno,_,_,_),forall(curso(Alumno,_,Nota,_), Nota >= 8).

seLoQueHicisteElVeranoPasado(Alumno):-anioCursada(Alumno,_,_,_),forall(anioCursada(Alumno,_,Anio,_),
...
materiasAnuales(Alumno,Total):-findall(Nota,curso(Alumno,_,Nota,anual(_)),Notas), sumlist(Notas,Total).
