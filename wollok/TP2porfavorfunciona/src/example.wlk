...
class Mensajeras{
	...
	method costoMensaje(mensaje){
		...
		const costoSector= sector.calcularCosto(cantidadPalabras) 
		return costoSector + mensaje.costoMensaje(costoSector)
...
class MensajeroAlegre inherits Mensajeras{
	...
	override method transformarMensaje(unMensaje){
		if(gradoAlegria > 10){
			const mensajeNuevo = new MensajeCantado(mensaje = unMensaje,duracion = self.duracion())
			return mensajeNuevo
...
object agenciaDeMensajeria{
	...
	method aQuienPedirle(mensaje){
		var mensajerosPosibles=mensajeros.filter({unMensajero=>unMensajero.puedeEnviar(mensaje)})
		return mensajerosPosibles.min({unMensajero=>unMensajero.costoMensaje(unMensajero.transformarMensaje(mensaje))})
...
class MensajeCantado inherits MensajeComun{
	...
	override method costoMensaje(costo){
		return costo*0.10
	}
